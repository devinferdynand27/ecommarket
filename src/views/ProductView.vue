<template>
  ...
  <div v-if="getProduct.products == undefined">
    <center>
      <br /><br /><br /><br />
      <div role="status">
        <svg
  ...
  <div v-else>
    ...
    <div class="bg-white">
      <div class="mx-auto max-w-1xl px-4 py-2 lg:max-w-7xl lg:px-7">
        <div 
          class="mt-1 grid grid-cols-1 gap-x-6 gap-y-9 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-5"
        >
          <div
            class="card shadow-xl group relative"
            v-for="product in getProduct.products.data"
            :key="product.id"
          >
            <router-link :to="'/product/detailproduct/' + product.slug">
              <div
                card
                shadow
              ...
              <div class="mt-4 flex justify-between">
                <div>
                  <h3 class="text-sm text-gray-700 pl-2 font-mono text-xl">
                    <a href="#">
                      <span aria-hidden="true" class="absolute inset-0"></span>
                      {{ product.name }}
                    </a>
                  </h3>
                  <p class="mt-1 text-sm text-gray-500 pl-2">
                    stok : {{ product.stock }}
                  </p>
                  <!-- <p>stok varians {{product.variations[0].stock}}</p> -->
                </div>
                ...
                <center>
                  <div
                    ...
                    data-te-chip-init
                    data-te-ripple-init
                    class="[word-wrap: break-word] my-[8px] mr-4 flex h-[30px] cursor-pointer items-center justify-between rounded-[16px] border border-[#f66c22] bg-[#eceff1] bg-[transparent] px-[12px] py-0 text-[13px] font-normal normal-case leading-loose text-[#4f4f4f] shadow-none transition-[opacity] duration-300 ease-linear hover:border-[#dc4c64] hover:!shadow-none dark:text-neutral-200"
